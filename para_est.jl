...
vv = collect(range(0,stop=10,length=400))
...
aa,bb,x_hat,y_hat,ee,ff=Rbff(vv)

for i in range(1,stop=length(vv))
  data[1,i]=x_hat[i]
  data[2,i]=y_hat[i]
end


cost_function = build_loss_objective(prob,A(0,0),L2Loss(vv,data),maxiters=10000,verbose=false)
